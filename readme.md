...
*[#13]§13 磁力效果
[/index]
[index]阅读时请注意内容的适用版本，有任何错误和疑问请联系我，谢谢！[/index]
[index]由于论坛原因，本帖中的代码直接复制可能会有特殊的字符导致无法使用！[/index]
...
### §1.2 工具准备
#### 文本编辑器
注意函数文件(.mcfunction)、JSON文件(.json)均需使用 UTF-8 编码格式，以下列出的文本编辑器默认均为 UTF-8 编码，可以在编辑页面的右下角状态栏看到。
...
#### 压缩工具
主流的压缩工具有付费软件 winrar 和免费软件 [7-zip](http://www.7-zip.org/)，Windows10 系统下可以直接打开 zip 格式文件。Minecraft 可以识别文件夹格式或 zip 格式的资源包和数据包。

#### nbt工具
推荐使用 [nbtexplorer](http://www.mcbbs.net/thread-735265-1-1.html) 来打开 dat 和其它 nbt 格式文件。

### §1.3 文件类型
我们简要介绍下将接触的文件类型，本段中不做详解，仅做眼熟用。为便于查看文件后缀，请将`文件->文件夹选项->查看->隐藏已知文件类型的扩展名`选项去掉勾选。其中前三者为纯文本文件，可通过新建文本文件然后修改后缀来创建，使用文本编辑器打开和编辑。
...
### §1.4 文件结构
打开.minecraft所在的文件夹，这通常位于你的启动器目录下。如果启动器中设置为各版本独立，则类似的文件结构位于`versions/版本号`下。

+ **assets** minecraft的资源文件。可在此找到所有游戏内翻译文本的键和翻译，之后便可通过加载资源包来修改。打开 indexes 中对应的 JSON 文件，查找 zh_cn 对应的 hash 值，然后在 objects 中找到相应文件，用文本编辑软件打开即可看到相应的文本。
...
+ **saves** 地图存档，我们从展示共享版下载的地图一般就是放入该文件夹。
    + **advancements** 玩家的进度和完成时间。
    + **playerdata** 玩家的属性等内容，可修改玩家的位置、复活点、飞行速度等内容。
    + **stats** 玩家的统计信息。
    + **data** 地图、记分板、村庄等内容。idcounts.dat记录了当前已使用的地图数量，修改后新打开的空地图会以此开始计数，这可以避免模组自定义的地图被玩家无意修改。map_数字.dat即相应的地图信息。scoreboard.dat记录了记分板、记分板的值、组。其它的文件记录了相应结构(村庄、要塞、废弃矿井)的信息。
    + **datapacks** 数据包文件夹，每个子文件夹或zip对应一个数据包。
    + **DIM1、DIM-1、region** 末地、下界和主世界的区块信息。.mca文件可用nbtexplorer打开并修改相应的区块内容，但是极为不便。建议使用mcedit等工具来编辑。
        ...
        data
            命名空间
                ...
                tags
                    ...
                    entity_types
```
如果你是首次接触数据包，请**务必将此文件结构掌握**。除数据包名称外，所有文件和文件夹请使用小写英文、数字或-,_来命名，**不可使用大写字母**，所有文本文件使用 UTF-8 编码。数据包可以用文件夹格式或 zip 压缩文件格式。发布时可将所有内容压缩为一个zip文件，压缩时注意文件夹层次以避免多套了一层文件夹。
...
进度、战利品表、标签、模型、语言文件均为JSON格式文件。JSON文件包括一对根括号{}，包含类似 "abc":"def" (键:值)的对。相同的键后者会覆盖前者。Minecraft 使用严格的JSON，键为需引号圈住的字符串，值可以为

+ 布尔型 例如 "is_on_fire": false 等。只能是true和false。
+ 数值 例如 2.0,1.8,30 等。
+ 字符串 例如 "def"，"minecraft:diamond" 等。注意需要用单引号或双引号圈住，可以使用颜色代码如§6、换行符\n。
+ 列表 例如 [1.0,2.0,3.0]，[{"type": "item","name": "minecraft:apple"},{"type": "loot_table","name": "minecraft:blocks/oak_leaves"}] 等。注意需要用方括号圈住。
+ JSON对象 例如 {"a":1s,"b":2.0d}，{"type": "item","name": "minecraft:apple"} 等。注意需要用花括号圈住。
编写时，建议通过空格或制表符(Tab)缩进，以便于查看括号匹配和层次。.mcmeta文件也是JSON文件，所以格式是相同的。wiki上有关页面使用了NBT的数据类型标注，但其实并不适用于JSON格式。
...
```
`pack_format` 必须为4。在输入 `/datapack list` 命令后，鼠标移至数据包名称上会显示 `description` 信息。Minecraft 通过该文件来识别数据包，因此该文件是不可或缺的。

### §2.2 命名空间
命名空间(namespace)为玩家自定义的、可操作的空间。如果不同数据包中有相同的命名空间，则其中相同的文件名内容会根据加载先后顺序被覆盖。特别地，原版内容被保存在minecraft命名空间，想要修改和替换原版的内容只需在你的数据包内建立minecraft命名空间和相应的同名文件并修改即可。注意tags中的文件内容默认追加而不是覆盖。

**进度、战利品表、函数、结构、标签、模型、材质等文件的调用格式均为`命名空间:文件夹/文件名`对应`命名空间/advancements/文件夹/文件名.后缀`等类似文件，其中的 advancements、loot_tables、functions、strutures、tags/blocks、tags/items、models、textures 的文件夹层次均无需写出。若命名空间为 minecraft，则可直接使用格式`文件夹/文件名`。**
...
注意部分内容随着版本会有所变化，请尽量以 wiki 和游戏实测为准。

进度(advancements)文件为JSON文件，后缀为json。进度为游戏内检测玩家行为，触发后完成进度并执行奖励的系统。游戏内按 L 即可查看。
该文件夹(或子文件夹)下每一个root.json文件都对应一个进度页面选项卡，因此可通过文件夹来分离不同的选项卡。
...
```这个进度的例子的图标为胡萝卜钓竿的自定义模型数据为10的模型，标题为翻译文本advancements.cpp.root.title，描述为翻译文本advancements.cpp.root.description，样式为圆形边框，背景为资源包中cm命名空间下的bg.png文件。检测条件为玩家背包含有tag:{id:"cpp:dream_wand"}的物品，奖励为函数cpp:adv/root和200点经验，并解锁配方"minecraft:crafting_table"和"cpp:strange_block"。

各项内容具体含义为
+ display 进度的样式，若无，则该进度不会出现在进度页面。
    + icon 进度的图标。
    + item 图标物品名称。
    + nbt 图标物品nbt，物品的材质模型可能会根据nbt不同而不同。
+ title 进度的名称，可以是单个的字符串或一个raw json对象，样例中translate文本，会随着语言改变的文本，文本内容记录在资源包内。
+ description 进度的描述，和进度的名称格式相同。
+ frame 进度的类型，分为task(任务 方形边框)，challenge(挑战 尖形边框)，goal(目标 圆形边框)。
+ background 仅根进度有，决定该选项卡的背景图，值为资源包内图片，可以为minecraft自带或其它资源包。
+ parent 除根进度外的进度，记录其父进度。
+ criteria 判据，其下为判据名称，触发器和触发器的条件。判据可以有多个，
+ requirements 判据的条件格式。不写时默认需要所有条件满足。格式为一些或的与(合取范式)，例如 "requirements": [["a","b"],["c","d"]]，当a和b至少有一个满足且c和d至少有一个满足时，会成功触发。
+ rewards 奖励

    + function 奖励函数，完成进度时执行之。若在函数中剥夺该玩家该进度，则该进度可反复触发。
    + recipes 奖励配方，通常检测玩家拥有配方材料后触发奖励配方。
    + experience 奖励经验，建议仅在高难度进度完成时给予。

进度配合奖励函数剥夺玩家该进度，可用于诸如检测玩家生物群系、饮食等需要循环检测的情形。这里要注意，对于触发器consume_item，即食用或饮下物品，如果对应物品为药水，并不能检测相应药水中的自定义nbt，例如"nbt": "{id:\"cpp:some_drink\"}"这种将不能被检测(除非药水能堆叠)。然而，"nbt": "{CustomPotionEffects:[{Id:99b,Duration:3600}]}"这种是可以被检测到的，尽管并没有Id:99b对应的药水效果。

### §2.4 函数
函数(functions)的文件后缀为mcfunction，每一行表示一个单独的命令，且无需/开头。执行时会按顺序依次执行，使用#开头的行为注释。游戏内或函数文件中可使用诸如
```
function cpp:somefoulder/somefunction```来依次执行函数的每一条命令。

### §2.5 战利品表
可参考
+ (http://www.mcbbs.net/thread-619468-1-1.html][CBL∫2b]Loottable - 创造一个看脸讲玄的世界 总索引]
+ (http://www.mcbbs.net/thread-831542-1-1.html]【CBL｜SPG】［1.14］战利品表：从入门到重新入门]
+ (https://minecraft-zh.gamepedia.com/%E6%88%98%E5%88%A9%E5%93%81%E8%A1%A8]战利品表 - Minecraft Wiki，最详细的官方我的世界百科]

战利品表(loot_tables)文件为JSON文件，后缀为json。进度用于实体的掉落物、钓鱼和箱子的随机物品。
...
```这个例子表示：牛死亡时掉落0-(2+抢夺等级)个皮革，1-(3+抢夺等级)个牛肉，着火则掉落熟牛肉。被玩家杀死时，有(2.5+抢夺等级)%几率掉落战利品表cpp:limb_of_ridge的物品，有(2.5+抢夺等级)%几率掉落牛头。

在minecraft命名空间下，记录了所有实体的掉落物、钓鱼和箱子战利品的战利品表，读者可通过jar文件查看其文件结构。若要修改之，在数据包中创建minecraft命名空间，并创建相应文件夹和JSON文件并修改之即可覆盖。
...
summon bat ~ ~ ~ {DeathLootTable:"cpp:chests/something"}
```在1.14中我们还可以使用loot命令来讲战利品表的物品输入到实体或容器的背包或槽位，或者直接掉落在地面上。

### §2.5 配方
配方(recipes)文件为JSON文件，后缀为json。配方包括合成(工作台/切石机)配方和烧炼(熔炉/高炉/烟熏炉/营火)配方，[color=red]配方不支持带NBT标签[/color]，想要实现带NBT的合成需要藉由其它方式。，具体见(https://www.mcbbs.net/forum.php?mod=viewthread&tid=784662&cp=5]合成]章节。

数据包中位于minecraft命名空间下的、与原版配方文件名称和位置相同的配方，会覆盖默认配方。

在默认情形(限制配方关闭)下，玩家总是可以使用所有的配方，但是未获得的不会在配方书中显示，因此建议为你的自定义配方添加合适的进度来获取该配方。

### §2.6 结构
可参考
+ (http://www.mcbbs.net/thread-652937-1-1.html]建筑党也能愉快享用结构方块-图文并茂教会你使用结构方块]
+ (http://www.mcbbs.net/thread-801350-1-1.html]如何使用结构方块]
+ (https://minecraft-zh.gamepedia.com/结构方块]结构方块 - Minecraft Wiki，最详细的官方我的世界百科]
结构(structures)文件为NBT文件，后缀为nbt。结构存储了一个长方体区域的方块和实体信息，玩家可以创建、保存、调用这样的结构。在游戏中获得结构方块(structure_block)后，放置好所需要的方块结构和实体，然后放置结构方块，设置为保存模式，调整好大小，输入名称，点保存即可（注意区分是否需要实体）。然后从generated文件夹复制到数据包内，在结构方块中输入"命名空间:文件夹/文件"格式加载（注意区分是否需要实体）。
我们也可以在函数中加载，先放置结构方块，然后放置并清除红石块即可。
**例** 
```
...
### §2.7 标签
标签(tags)文件为JSON文件，后缀为json。其中的子文件夹分为
+ **blocks** 为方块标签，在命令中用于检测/调用多个方块，调试模式(F3)下，玩家指向方块时会显示含有该方块的所有方块标签；
+ **items** 为物品标签，在clear命令中用于清除多个不同id物品，在配方文件中表示合成/熔炼时所需的物品可以为标签中的任一种，也可用于进度判定；
+ **functions** 为函数标签，在function命令中一次性按次序执行多个函数；
+ **entity_types** 为实体类型标签，在选择器@e的选项type中指定多种不同id的实体，例如@e[type=#cpp:hostiles]；
+ **fluids** 为流体标签。
**例** 当玩家眼睛位置不是流体(空气、水等)时，输出I'm stucked!
...
```
这里的 `replace` 表示是否覆盖原标签内容，默认为 `false`，因此这一行可以省略。如果设置为 `true`，则会覆盖之前原版和其它数据包添加至该标签的内容。

然后执行命令
```
execute as @a at @s anchored eyes unless block ^ ^ ^ #cpp:fluid run say I'm stucked!
```
因为命令执行地点为实体的脚，所以我们使用anchored eyes来使得局部坐标的位置变为玩家的眼睛。
...
```
tags中亦可引用其它tags。
**例** minecraft/tags/blocks/logs.json
```
{
    "values": [
...
#### 预设标签
minecraft命名空间中预设了一些tags。其中

+ `minecraft/tags/functions/load.json` 中的函数会在加载时被执行一次，我们常称之为加载函数、load函数。
+ `minecraft/tags/functions/tick.json` 中的函数每刻会被执行一次，我们常称之为循环函数、主函数、tick函数。

**例** minecraft/tags/functions/tick.json
```
{
    "values": [
        "cpp:tick",
...
```利用预设标签可以实现更多燃料、或者改变植物种植所需的方块，甚至改变水和熔岩的性质，可参见(http://www.mcbbs.net/thread-811990-1-1.html]minecraft预设tag以及实现随机方块功能]。
        ...
        assets
            命名空间
                lang
                    ...
                    en_us.json
                models
                textures```其中assets的文件结构和§1.4中版本.jar/assets的结构是一样的。除资源包名称外，所有文件和文件夹请使用小写英文、数字或-,_来命名，[color=red]不可使用大写字母[/color]，所有文本文件为使用UTF-8编码的JSON文件，图片使用png格式。

在资源包菜单中，靠上的资源包内的文件会覆盖靠下的资源包中的同名文件。由于原版模组往往会修改原版物品模型，因此同时安装多个原版模组的资源包时，很容易出现文件冲突，此时需要玩家手动合并资源包信息方可使用。在存档内的resources.zip资源包会在进入存档时自动加载，且会覆盖其它已加载的资源包。

本文中我们仅对资源包做简单介绍，更详细的文件说明请参阅(https://minecraft-zh.gamepedia.com/%E8%B5%84%E6%BA%90%E5%8C%85]资源包 - Minecraft Wiki，最详细的官方我的世界百科]。

### §3.1 pack.mcmeta和pack.png
例如
```
{
    "pack": {
...
```pack_format必须为4。Minecraft通过该文件来识别资源包，因此该文件时必需的。

可包含一个pack.png，大小为16*16的图片，用于在资源包菜单中显示。在资源包菜单中，会显示资源包名称和图片。

### §3.2 命名空间
命名空间(namespace)为玩家自定义的、可操作的空间。如果不同资源包中有相同的命名空间，则其中相同的文件名内容会根据加载先后顺序被覆盖。特别地，原版内容被保存在minecraft命名空间，想要修改和替换原版的内容只需在你的数据包内建立minecraft命名空间和相应的同名文件并修改即可。

[color=red]模型、材质等文件的调用格式均为"命名空间:文件夹/文件名"对应"命名空间/models/文件夹/文件名.后缀"等类似文件，其中的models、textures的文件夹层次均无需写出。若命名空间为minecraft，则可直接使用格式"文件夹/文件名"。[/color]
...
The resourcepack does not Installl coreectly or Launch.```以提示玩家资源包未加载。
...
模型本身只是一个JSON文件，只有minecraft命名空间下特定名称的文件才对应特定的物品/方块模型。例如minecraft/models/item/carrot_on_a_stick.json表示胡萝卜钓竿的物品模型，minecraft/models/block/stone.json表示石头的方块和物品模型。材质文件也是类似的。
...
elements中的from和to确定长方体区域。我们放置方块时，离我们最近的为方块的为方块的北方(north)，然后站在方块的视角按前北后南(z轴)，左西右东(x轴)确定其6个面的朝向。faces为方块的6个面，我们建议按次序东北西南上下的次序写以便于想象。可以在材质文件中按照

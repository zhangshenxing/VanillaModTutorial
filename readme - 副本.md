...
`tick.mcfunction`
```
execute as @a[scores={cppUseCSt1=1..}] at @s run function cpp:tool/elements/use
```
`cpp/functions/tool/elements/use.mcfunction`
```
execute as @s[nbt={SelectedItem:{tag:{id:"cpp:blue_force_of_sky"}}}] positioned ~ 1000 ~ run function cpp:tool/elements/blue
execute as @s[nbt=!{SelectedItem:{id:"minecraft:carrot_on_a_stick"}},nbt={Inventory:[{Slot:-106b,tag:{id:"cpp:blue_force_of_sky"}}]}] positioned ~ 1000 ~ run function cpp:tool/elements/blue
scoreboard players reset @s cppUseCSt1
```
`cpp/functions/elements/use/blue.mcfunction`
```
...
```
`cpp/loot_tables/misc/weather.json`
```
{
	"pools": [
		{
...
`tick.mcfunction`
```
execute as @a[scores={cppUsePearl=1..}] at @s anchored eyes positioned ^ ^ ^ run function cpp:tool/transport
execute as @a[tag=cpp_transport_ball_player] at @s run fun ction cpp:tool/tp
```
`cpp/functions/tool/transport.mcfunction`
```
tag @s[nbt={SelectedItem:{tag:{id:"cpp:transport_ball"}}}] add cpp_transport_ball_player
tag @s[nbt=!{SelectedItem:{id:"minecraft:carrot_on_a_stick"}},nbt={Inventory:[{Slot:-106b,tag:{id:"cpp:transport_ball"}}]}] add cpp_transport_ball_player
scoreboard players set @s cppUsePearl 0
loot give @s loot cpp:transport_ball
```
`cpp/functions/tool/tp.mcfunction`
```
tp @s @e[type=ender_pearl,distance=..5,nbt={Item:{tag:{id:"cpp:transport_ball"}}},limit=1]
execute unless @e[type=ender_pearl,distance=..5,nbt={Item:{tag:{id:"cpp:transport_ball"}}}] run tag @s remove cpp_transport_ball_player
```
